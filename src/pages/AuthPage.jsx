...
export default function AuthPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      ...
      if (result.success) {
        // ÚJ LOGIKA: Ha a rangod Admin (kis- vagy nagybetűvel is működik)
        if (result.role && result.role.toLowerCase() === 'admin') {
           navigate('/admin'); // Átirányítás a titkos Admin oldalra!
        } else {
           navigate('/'); // Sima vásárló visszamegy a főoldalra
        }
      } else {
